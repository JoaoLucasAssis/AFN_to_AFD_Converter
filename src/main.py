...
def validar_palavras(afd, palavras):
    ...
    for palavra in palavras:
        ...
        for caractere in palavra: # ['1', '0', '0', '1']
            if caractere in afd['transicoes'][estado_atual]: # {'A': {'1': ('A', 'B', 'C'), '0': ('B', 'C')}}
                estado_atual = afd['transicoes'][estado_atual][caractere] # ('A', 'B', 'C')
            else:
...
def ler_palavras(caminho_arquivo): 
    with open(caminho_arquivo, 'r') as f:
        linhas = f.readlines()
    return [list(linha.strip()) for linha in linhas] # [['0', '1', '0'], ['1', '0', '0', '1']]
...
def obter_estados_alcancaveis(afnd, estados):
    ...
    while fila:
        ...
        if estado_atual in afnd['transicoes'] and 'h' in afnd['transicoes'][estado_atual]: # {'A': {'1': ['A', 'B'], 'h': ['B', 'C']}}
            for estado in afnd['transicoes'][estado_atual]['h']: # ['B', 'C']
                if estado not in estados_alcancaveis:
                    estados_alcancaveis.add(estado)
                fila.append(estado)
    return tuple(sorted(estados_alcancaveis))  # Dicionário aceita apenas tuplas como chaves ('B', 'C')

def calcular_transicoes_para_estado(afnd, estado_atual):
    transicoes_por_simbolo = defaultdict(set)
    
    for estado in estado_atual: # ('A', 'B')
        if estado in afnd['estados']:
            for simbolo, proximos_estados in afnd['transicoes'][estado].items(): # {'A': {'1': ['A', 'B'], 'h': ['B', 'C']}}
                if simbolo != 'h': # {'1': ['A', 'B']}
                    for proximo_estado in proximos_estados: # ['A', 'B']
                        transicoes_por_simbolo[simbolo].update(obter_estados_alcancaveis(afnd, [proximo_estado])) # ('A', 'B', 'C')
...
def converter_afnd_para_afd(afnd):
    ...
    while nao_processados:
        ...
        # Cálculo do novo estado através das transições do símbolo
        transicoes_por_simbolo = calcular_transicoes_para_estado(afnd, estado_atual) 
    
        for simbolo, novo_estado in transicoes_por_simbolo.items():
            novo_estado = tuple(sorted(novo_estado))  # Dicionário aceita apenas tuplas como chaves ('A', 'B', 'C')
...
def ler_afnd(caminho_arquivo):
    ...
    for linha in linhas[3:]:
        ...
        afnd['transicoes'][estado_atual][simbolo].append(proximo_estado) # {'A': {'1': ['A', 'B'], 'h': ['B', 'C']}}
